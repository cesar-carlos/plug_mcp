...
- Consulta semântica: se buscar_contexto devolver consultaSemanticaSugerida (KPI certificado com maior overlap da pergunta), prefira consultar_dados.consultaSemantica com esse esqueleto. SQL livre só se faltar elemento certificado. Colunas são qualificadas quando há JOIN.
- PERFIL_AUSENTE bloqueia inferência e a primeira publicação; não invente tipo, dicionário, grão ou JOIN. CONSULTA_ORCAMENTO respeita politicaConsulta da skill. Pacote mínimo (fluxoTreino.pacoteMinimo): uma tabela, colunas nomeadas, WHERE ou agregação, params com descricao; JOIN/KPI só se o usuário pedir. Skill validada com perfil incompleto: listar_skills.faltas[] e fluxoTreino.proximoPasso nunca são nulos — chame a nextAction (confirmar_relacionamento, mapear_tabela, listar_conflitos).
