...
- Consulta semântica: se buscar_contexto devolver consultaSemanticaSugerida (KPI de agregação com maior overlap da pergunta; CAST/data não entram), prefira consultar_dados.consultaSemantica com esse esqueleto. SQL livre só se faltar elemento certificado. Colunas são qualificadas quando há JOIN.
...
- buscar_contexto: cobertura certificada (nome/slug/descrição/params/metricasSaida, não o SQL nem o corpo da regra). conhecimentos[] é evidência FTS/ILIKE (não embeddings/RAG); stem une inflexão na cobertura. Não invente tabela/JOIN a partir deles e só chame consultar_dados se consultaPermitida. Envelope sem sqlModelo nem SQL aprendido; use consultasAprendidas[].id em obter_skill.consultasExemplo. Cobertura parcial: obter_skill e validar_consulta; registrar_aprendizado tipo=sinonimo se o usuário confirmar o termo. Se consultaPermitida e houver KPI de agregação (não CAST), use consultaSemanticaSugerida (maior overlap da pergunta). Skill em treino que cobre a pergunta → blockingReason SKILL_NOT_PUBLISHED (não é SKILL_GAP). Sem skill capaz: SKILL_GAP (não registre sinônimo; não cruze skills sem JOIN publicado; fluxoTreino só se houver skill em andamento que cubra a pergunta) e, se faltar tool, registrar_lacuna_ferramenta. listar_conflitos devolve ids para resolver_conflito.
...
- columns/rows/columnsMetadata: tipos JS string/number/boolean/null; datas como string ISO. Cite sqlExecutado, asOf, recorte e escopoAplicado. Zero linhas ainda traz colunas/metadata. inspecionar_consulta devolve o mesmo columnsMetadata (tipos/nulidade); PII só em rows.
...
- avisos[].code são sinais a agir: LITERAL_TEXTO, ESCOPO_CONSOLIDADO, TIMEZONE_INVALIDO, PLACEHOLDER_ESCOPO, PERFIL_AUSENTE, CACHE, KPI_DESALINHADO, SCHEMA_DRIFT. REGRA/METRICA só da skill da chamada ou da tabela do SQL (teto curto de REGRA; globais de processo ficam em obter_skill).
