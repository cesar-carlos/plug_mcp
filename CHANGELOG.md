...
- Catálogo de treino (skill, grafo, anotações, consultas aprendidas, sinônimos, lacunas, FTS) passa a ser por `acesso_id` — **1 `client_token` = 1 persona = 1 catálogo**. Unique slug `(acesso_id, slug)`. Mesmo e-mail/`agentId` + outro token (`adicionar_acesso`) começa vazio e não vê as skills do primeiro. N=1: tools omitem `acessoId`. N>1: passe `acessoId`; `skillId` ou slug **único** nos catálogos do usuário amarra o acesso (`skill_*` / `obter_skill` / `consultar_dados`) — catálogos **não** se unem; ambíguo ou ausente → `VALIDATION_ERROR`. Com N>1, o nome da tool `skill_*` inclui sufixo do `acessoId` (não só em clash de slug), para a lista MCP não parecer um catálogo só. `exportar_anexo` infere o acesso do handle se ele for deste usuário. `listar_auditoria` / `listar_metricas_agente`: N=1 amarra; N>1 exige `acessoId` (não despeja o SQL history da outra persona). Resource `skill://{acessoId}/{slug}` (não `agentId` na URI). Cache `mcp:query:acesso:{acessoId}:`. Hub SQL continua `agentId` + `client_token` daquele acesso. Cutover `0022_catalogo_por_acesso.sql`: um acesso no `agentId` anexa; vários duplicam cópias independentes; órfãos (zero acessos) ficam com `acesso_id` NULL e são registrados em `NOTICE` (`grafo_dialeto`/`grafo_lock` apagam órfãos).
...
- `notifications/tools/list_changed` só acorda sessões **deste** `usuarioId` (não quem só compartilha `agentId`).
