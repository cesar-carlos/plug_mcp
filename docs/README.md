...
| [`data/`](data)                 | [data-model.md](data/data-model.md)                                                                                                                                             | Cofre, grafo composto, FTS (`0016`–`0019`, **não** RAG), skill v2, aprendizado (`0020`), persona (`0021`), catálogo por acesso (`0022_catalogo_por_acesso`) |
...
1. [product/objective.md](product/objective.md) — invariantes: SQL no plug-server, dialeto do acesso (identificar o GDBR e emitir SQL compatível é treino + IA; o hub não reescreve dialeto), resources `guia://` (bootstrap), `skill://{acessoId}/{slug}` e `persona://` (Bearer); **1 `client_token` = 1 persona = 1 catálogo isolado** (`acesso_id`); N=1 omita `acessoId`, N>1 passe (`skillId`/slug único amarra; `listar_auditoria` exige `acessoId`); papel = persona (tom) + skills (pacote); várias personas = vários acessos; fail-closed; sem embeddings.
