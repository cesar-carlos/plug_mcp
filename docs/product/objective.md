...
O validador recusa tabela, coluna, alias, UNION/subquery ou JOIN fora do escopo das skills publicadas informadas. `buscar_contexto` devolve `cobertura` (`completa` | `parcial` | `desconhecida`) e `consultaPermitida` só quando a cobertura é completa. Envelope sem `sqlModelo` nem SQL de `consultasAprendidas` — reuse o `id` em `obter_skill`. Se `consultaPermitida` e houver KPI, `consultaSemanticaSugerida` (escolhe o alias certificado com maior overlap da pergunta; empate: IR persistido, depois ordem; prefira `consultar_dados.consultaSemantica`). Cobertura parcial: `registrar_aprendizado tipo=sinonimo` se o usuário confirmar o termo. Cobertura de capacidade usa nome, slug, descrição, params e `metricasSaida` — não o SQL nem o corpo da regra. `conhecimentos[]` é evidência **FTS/`ILIKE`** (não RAG, não embeddings); stem une inflexão na cobertura; similaridade **não** autoriza consulta. `SKILL_NOT_PUBLISHED` só se a skill em treino cobre a pergunta. `SKILL_GAP` da busca por termos não prova ausência — `listar_skills` antes de desistir. Se houver rascunho/validada/`rascunho_revalidacao` **capaz**, o hint pede para continuar o `proximoPasso` (`faltas[].nextAction` quando publicar está bloqueado).
