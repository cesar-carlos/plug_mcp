...
5. `SKILL_GAP` vira `lacuna_consulta` só quando a busca é específica o bastante. `buscar_contexto` devolve `consultasAprendidas` e `conhecimentos[]` (evidência; não autoriza consulta) para a IA reusar em vez de reinventar.
