...
- Loop de aprendizado: `buscar_contexto` devolve perguntas de `consultasAprendidas` (status `ativa`, sem SQL); o SELECT está em `obter_skill` (`consultasExemplo`). Sinônimos resolvem skill por id/slug/nome sem concatenar UUID na tsquery. Lacunas e promoção a `validado_execucao`. `asOf` usa o timezone do acesso.
...
- `buscar_contexto` não devolve `sqlModelo` nem SQL aprendido; `conhecimentos[]` é evidência, não licença.
