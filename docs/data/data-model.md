...
- `consulta_aprendida`: SQL que funcionou (gravado automaticamente em `consultar_dados`), pergunta, contrato de params, contagem de execuções, `status` (`ativa` | `inativa`). Associação N:N em `consulta_aprendida_skill`. Ao remover a skill, os vínculos somem; o SQL histórico permanece no `agentId`. `obter_skill.pacote.consultasExemplo` devolve as da skill. `buscar_contexto` devolve só perguntas de consultas `ativa` (sem SQL); o SELECT está em `obter_skill`.
