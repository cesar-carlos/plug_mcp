...
- Busca léxica (migration `0016_conhecimento_fts.sql`): coluna gerada `search_tsv` + GIN em `tabela_grafo` (nome, descrição), `skill` (nome, slug, descrição, params, definições de `metricasSaida` — não dump cru do JSON nem `sql_modelo`), `anotacao_grafo` (título+texto), `consulta_aprendida` (**só** `pergunta`). Extensão `unaccent` via função IMMUTABLE `mcp_unaccent`. **Não** se indexa SQL aprendido, linhas de ERP nem amostras de inspeção. Fallback `ILIKE` para identificadores. O dicionário FTS é `portuguese` do Postgres (stopwords distintas das de `coberturaDeSkill`).
